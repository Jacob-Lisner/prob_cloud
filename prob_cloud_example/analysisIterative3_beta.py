...
def main():
    ...
    method = MLPRegressor(hidden_layer_sizes = (200,100,25), activation = "tanh", solver = "adam", learning_rate = "adaptive", random_state = 0)
    cloud = prob_cloud.cloud(method, pred_type = "best")

    xVals = xFeed
    yVals = yfdata
    cloud.fit(xVals,yVals)

    confs = cloud.predict(xVals)
    fig = plt.figure()
    ax = fig.add_subplot()
    i = 0
    best = mean_squared_error(confs,yfdata)
    print(best)
    for i in range(2, xdata.shape[1]):
        ...
        pred = cloud.predict(xFeed)
        if(mean_squared_error(pred,yfdata) < best):
            confs = pred
            best = mean_squared_error(pred,yfdata)
        print(best)
        if(i%10 == 0):
